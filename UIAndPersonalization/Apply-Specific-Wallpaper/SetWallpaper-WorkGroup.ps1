...
# 7. Update registry hive for that user
...
# 8. Ensure wallpaper refreshes every logon
$runKeyPath = "Registry::HKEY_USERS\$sid\Software\Microsoft\Windows\CurrentVersion\Run"
$runEntryName = "ReapplyWallpaper"
$runEntryValue = "rundll32.exe user32.dll,UpdatePerUserSystemParameters"
New-Item -Path $runKeyPath -Force | Out-Null
Set-ItemProperty -Path $runKeyPath -Name $runEntryName -Value $runEntryValue

# 9. Create scheduled task to refresh wallpaper once now
...
Write-Host "Log out and log back in to apply the wallpaper change and make it persistent."
